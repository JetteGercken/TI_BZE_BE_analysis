...
BZE3_trees <- rbind(
  ...
  BZE3_trees %>% filter(tree_inventory_status == 6) %>% mutate(tree_ID = 28, SP_code = "gki" , azi_gon = azi_gon +1, D_mm = D_mm+100, tree_inventory_status = 0)
  )
...
for (i in 1:length(BZE3_trees_9$tree_ID)) {
  ...
  tree_inventory_status_1 <- ifelse(!is.na(tree_inventory_status_2 & inv.status.nearest.neighbour !=1 ), 0, inv.status.nearest.neighbour)
...
BZE3_trees <- BZE3_trees %>% 
  ...
  mutate(new_tree_inventory_status = ifelse(tree_inventory_status == -9 & !is.na(tree_inventory_status_new) | is.na(tree_inventory_status) & !is.na(tree_inventory_status_new), tree_inventory_status_new, tree_inventory_status)) %>% 
...
for (i in 1:length(BZE3_trees_6$tree_ID)) {
  ...
  closest.id <- which.min(distance(x.tree.pre, y.tree.pre, x.tree.2, y.tree.2))
  
  # calculate or select the actual distance, species and dbh between the selected row/ coordiantes of tzhe nearest neighbout canidate from HBI and the given tree from BZE3
  distance.my.tree.and.nearest.neighbour.pre <- distance(x.tree.pre[closest.id], y.tree.pre[closest.id], x.tree.2, y.tree.2)
  ...
  # we can assume its the same tree and they just forgot to give a tree inventory status number if:
      ...
      # maybe also  dbh is lower or equal
      # if the tree id is identical
  my.tree.found.in.inv.1.dataset <- ifelse(distance.my.tree.and.nearest.neighbour.pre <= 0.5 & 
...
# find partner trees in the same inventory dataset (bze3, inventory 2, most recent inventory): 
  ...
  closest.tree.2.df <- closest.trees.df[which.min(abs(closest.trees.df$DBH_cm - dbh.nearest.neighbour.pre)),]
  ...
  tree_inventory_status_6.list[[i]] <- if(tree.found.in.inv.1.dataset == "yes"){
    as.data.frame(rbind(
      ...
      HBI_trees %>% filter(plot_ID == my.plot.id) %>% slice(closest.id) %>% 
        ...
        mutate(new_tree_inventory_status = ifelse(!is.na(closest.tree.2.df$new_tree_inventory_status & 
      ...
      BZE3_trees_6[i,] %>% mutate(old_tree_ID = tree_ID, 
                                  tree_type_status_6 = "my_tree_inv_2"),
