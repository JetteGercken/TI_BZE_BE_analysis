...
trees_stock_changes_P <- 
  ...
  full_join(BZE3_LT_summary %>% 
              select(stand_component, plot_ID, stand, stand_type, SP_code, compartiment, B_t_ha, C_t_ha, N_t_ha, n_ha, n_SP, 
                     mean_DBH_cm, sd_DBH_cm, Dg_cm, mean_BA_m2, mean_H_m, sd_H_m, Hg_m, n_stands) %>%
  ...
  mutate(across(contains("t_ha") | contains("stand_type") | contains("n_stands"), ~ifelse(is.na(.x), 0, .x)) )%>% 
