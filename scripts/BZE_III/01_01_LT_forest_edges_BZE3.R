...
trees_data <- read.delim(file = here(paste0(out.path.BZE3, "BZE3_LT_update_0_demo.csv")), sep = ";", dec = ",") %>% select(-c(stand, inter_stat, area_m2, stand_plot_A_ha, plot_A_ha, 
                                                                                                                              X_tree ,     Y_tree, e_ID, e_type ,e_form))
...
geo_loc <- geo_loc %>% dplyr::select(c("ï..ToTraktId", "ToEckId", "K2_RW",
                                       ...
                                       "LAT_MEAN", "LON_MEAN")) %>% mutate(inv = "BZE3", 
                                                                           inv_year = 2023)
colnames(geo_loc) <- c("plot_ID", "ToEckId", "K2_RW",
                       "K2_HW", "K3_RW", "K3_HW", "RW_MED",
                       "HW_MED",  "LAT_MED",  "LON_MED", 
                       "LAT_MEAN", "LON_MEAN", "inv", "inv_year") 
...
# ----- 1.1.2.1.1. join in forest edge info per plot -----------------------------------------------
...
# there are different ways how we can end up here: 
    # either there are no edges at all: nrow(forest_edges)  == 0
    # there may be no plots with 1 edge: nrow(forest_edges.man.sub.1.edge.nogeo) == 0 
    # there may be no plots with 2 edges: nrow (forest_edges.man.sub.2.edges.nogeo) == 0
# in that case, the inital loops for the edge polygones wont work 
    # or there are polygones but there are not trees inside them: 
       # (tree.status.two.edges.df.nogeo) == 0
      # (tree.status.one.edge.df.nogeo) == 0
      # in this case i could just filter for the edge area dataframe
      # we can assume that if there are no edges or no trees in the edges, the tree status dfs will end up empty
      # thus we should just anti join for the two separate tree status datasets
# because they should be an emty dataframe that is empty if the loop with the edges never run but also if the trees were not in the edges
      # we should figure out if it would actually not run, if there are no edges to process 
      
# in the first case, however we have to be able to filter 
