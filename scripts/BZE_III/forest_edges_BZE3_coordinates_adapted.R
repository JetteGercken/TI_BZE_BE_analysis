...
x_coord <- function(Dcp, AZIcp){ 
  # this is actually the y coordinate in an normal coordiante system
...
y_coord <- function(Dcp, AZIcp){ 
  # this is actually the x coorsinante in a normal coordinate system
...
# ----- 0.5.4. line functions -------------------------------------------------

# ----- 0.5.4.1. line functions -------------------------------------------------
slope <- function(x1, y1, x2, y2){
  # i have to change the order, because im working with coordiantes, so everything usually called y will have to be called x
  # b1 = (y2 - y1)/(x2 - x1);
   b1 = (x2 - x1)/(y2 - y1);
  return(b1)
}

# ----- 0.5.4.2. intercept y axis line -------------------------------------------------
...
intercept <- function(x1, y1, slope){
  ...
  # have to change the order, because its coordiantes, so everything usually called y will have to be called x
  # b0 = y1 - slope*x1;
  b0 = x1 - slope*y1;
  return(b0)
}

# ----- 0.5.4.3. linear function -------------------------------------------------
Y_line <- function(slope, intercept, y.coord.or.x.if.no.coord){
  y = intercept + slope*y.coord.or.x.if.no.coord
  return(y)
}



# ----- 0.5.4.4. implicit function ----------------------------------------
implicit <- function(slope, intercept, x, y){
  # here we transform the line equation: y = b0 + b1*x |-y --> 0 = b0 + b1*x - y
  # because we work with coordinates, we have to place y instead of x and x instead of y 
  # Y_impl <- intercept  + slope*x - y;
  Y_impl <- intercept  + slope *y - x;
  return(Y_impl)
}
...
# x1 of intersection --> called y1 in coordinate system
intersection_c_lx1 <- function(l.b0, l.b1, c.y0, c.x0, c.r0) {
 ...
 # c.r0 =1784;
  ...
  p = ((2*c.y0) + (2*l.b1*(l.b0 - c.x0)))/(1 + l.b1^2);
  q = (c.y0^2 + (l.b0 - c.x0)^2 - c.r0^2)/(1 +l.b1^2);
  x1 <-  -(p/2) + sqrt( ((p*-1)/2)^2-q );
  x2 <- - (p/2) - sqrt( ((p*-1)/2)^2-q );
  i.df <- as.data.frame(cbind(x1, x2));
...
azi_correction <- function(x2, y2, x1, y1, azi){
  delta_y = y2 - y1 ;
  delta_x = x2 - x1 ;
  azi_corrected = ifelse( delta_y >= 0  & delta_x >= 0 , azi,                              # first quadrant  coord_x + coord_y + | x+, y+
               ifelse( delta_y > 0  & delta_x < 0 , azi+200,                    # second quadrant coord_x - coord_y + | x+, y-
                      ifelse(delta_y < 0  & delta_x < 0  ,  azi+200,           # third quadrant  coord_x - coord_y - | x-,  y- 
                             ifelse(delta_y < 0  & delta_x > 0 , azi+400, NA)
                             )
                      )
               );
...
# find x of intercept with circle: insert line equation in circle equation
        # for AB line 
  mutate(Y_coord_X1_inter_AB =intersection_c_lx1(b0_AB, b1_AB,  data_circle$y0[3], data_circle$x0[3], data_circle$r0[3]),
         Y_coord_X2_inter_AB =intersection_c_lx2(b0_AB, b1_AB, data_circle$y0[3], data_circle$x0[3], data_circle$r0[3]), 
         inter_status_AB = ifelse(is.na(Y_coord_X1_inter_AB) & is.na(Y_coord_X2_inter_AB), " no I",      # if 0 solutions
                                  ifelse(Y_coord_X1_inter_AB == Y_coord_X2_inter_AB, "one I",            # if 1 solution
                                         ifelse(Y_coord_X1_inter_AB != Y_coord_X2_inter_AB, "two I"))),  # if 2 solutions
         # for AT line
         Y_coord_X1_inter_AT =intersection_c_lx1(b0_AT, b1_AT, data_circle$y0[3], data_circle$x0[3], data_circle$r0[3]),
         Y_coord_X2_inter_AT =intersection_c_lx2(b0_AT, b1_AT, data_circle$y0[3], data_circle$x0[3], data_circle$r0[3]), 
         inter_status_AT = ifelse(is.na(Y_coord_X1_inter_AT) & is.na(Y_coord_X2_inter_AT), " no I",     # if 0 solutions
                                  ifelse(Y_coord_X1_inter_AT == Y_coord_X2_inter_AT, "one I",           # if 1 solution
                                         ifelse(Y_coord_X1_inter_AT != Y_coord_X2_inter_AT, "two I"))), # if 2 solutions
         # for BT line
         Y_coord_X1_inter_BT =intersection_c_lx1(b0_BT, b1_BT, data_circle$y0[3], data_circle$x0[3], data_circle$r0[3]),
         Y_coord_X2_inter_BT =intersection_c_lx2(b0_BT, b1_BT, data_circle$y0[3], data_circle$x0[3], data_circle$r0[3]), 
         inter_status_BT = ifelse(is.na(Y_coord_X1_inter_BT) & is.na(Y_coord_X2_inter_BT), " no I",            # if 0 solution
                                  ifelse(Y_coord_X1_inter_BT == Y_coord_X2_inter_BT, "one I",                  # if 1 solution
                                         ifelse(Y_coord_X1_inter_BT != Y_coord_X2_inter_BT, "two I"))) ) %>%   # if 2 solutions
  # y intercept wih cirlce: insert x of intercept with circle in equation of line
         # AB line 
  mutate(X_coord_Y1_inter_AB = Y_line(b0_AB, b1_AB, Y_coord_X1_inter_AB), 
         X_coord_Y2_inter_AB = Y_line(b0_AB, b1_AB, Y_coord_X2_inter_AB), 
         # AT line 
         X_coord_Y1_inter_AT = Y_line(b0_AT, b1_AT, Y_coord_X1_inter_AT), 
         X_coord_Y2_inter_AT = Y_line(b0_AT, b1_AT, Y_coord_X2_inter_AT), 
         # BT line 
         X_coord_Y1_inter_BT = Y_line(b0_BT, b1_BT, Y_coord_X1_inter_BT), 
         X_coord_Y2_inter_BT = Y_line(b0_BT, b1_BT, Y_coord_X2_inter_BT)) %>% 
  # azimut of intersection points (calcaulte azimute with azimute function, then correct it depending on the quadrant of x_intersection and y_intersection)
         # AB line 
  mutate(azi1_inter_AB = azi_correction(X_coord_Y1_inter_AB, Y_coord_X1_inter_AB, 0, 0, azimut(X_coord_Y1_inter_AB, Y_coord_X1_inter_AB, 0, 0)), 
         azi2_inter_AB = azi_correction(X_coord_Y2_inter_AB, Y_coord_X2_inter_AB, 0, 0, azimut(X_coord_Y2_inter_AB, Y_coord_X2_inter_AB, 0, 0)),
         # AT line
         azi1_inter_AT = azi_correction(X_coord_Y1_inter_AT, Y_coord_X1_inter_AT, 0, 0, azimut(X_coord_Y1_inter_AT, Y_coord_X1_inter_AT, 0, 0)), 
         azi2_inter_AT = azi_correction(X_coord_Y2_inter_AT,Y_coord_X2_inter_AT, 0, 0, azimut(X_coord_Y2_inter_AT,Y_coord_X2_inter_AT, 0, 0)), 
         # BT line 
         azi1_inter_BT = azi_correction(X_coord_Y1_inter_BT, Y_coord_X1_inter_BT, 0, 0, azimut(X_coord_Y1_inter_BT, Y_coord_X1_inter_BT, 0, 0)), 
         azi2_inter_BT = azi_correction(X_coord_Y2_inter_BT,Y_coord_X2_inter_BT, 0, 0,  azimut(X_coord_Y2_inter_BT,Y_coord_X2_inter_BT, 0, 0)) ,
    # distance interception centre --> to see if points are actually placed on the rim of the circle 
         inter_1_dist = distance(X_coord_Y1_inter_AB, Y_coord_X1_inter_AB, 0, 0),     # this is just to control if the whole thing worked and 
    ...
    azi_T_AT_inter_1 = azi_correction(X_coord_Y1_inter_AT, Y_coord_X1_inter_AT, X_T, Y_T, azimut(X_coord_Y1_inter_AT, Y_coord_X1_inter_AT, X_T, Y_T)),
    azi_T_AT_inter_2 = azi_correction(X_coord_Y2_inter_AT, Y_coord_X2_inter_AT, X_T, Y_T, azimut(X_coord_Y2_inter_AT, Y_coord_X2_inter_AT, X_T, Y_T)),
     # BT line
    azi_T_B = azi_correction(X_B, Y_B, X_T, Y_T, azimut(X_B, Y_B, X_T, Y_T)),
    azi_T_BT_inter_1 = azi_correction(X_coord_Y1_inter_BT, Y_coord_X1_inter_BT, X_T, Y_T, azimut(X_coord_Y1_inter_BT, Y_coord_X1_inter_BT, X_T, Y_T)),
    azi_T_BT_inter_2 = azi_correction(X_coord_Y2_inter_BT, Y_coord_X2_inter_BT, X_T, Y_T, azimut(X_coord_Y2_inter_BT, Y_coord_X2_inter_BT, X_T, Y_T)),
    ...
    dist_T_AT_inter_1 = distance(X_coord_Y1_inter_AT, Y_coord_X1_inter_AT, X_T, Y_T), 
    dist_T_AT_inter_2 = distance(X_coord_Y2_inter_AT, Y_coord_X2_inter_AT, X_T, Y_T), 
    dist_T_BT_inter_1 = distance(X_coord_Y1_inter_BT, Y_coord_X1_inter_BT,  X_T, Y_T), 
    dist_T_BT_inter_2 = distance(X_coord_Y2_inter_BT, Y_coord_X2_inter_BT , X_T, Y_T), 
    ...
    Y_coord_X_inter_AT_triangle = case_when(T_dist <= 1784 &  azi_T_AT_inter_1 == azi_T_A ~ intersection_c_lx1(b0_AT,b1_AT,0,0, data_circle$rmax[3]*2),
                                    ...
                                    TRUE ~ NA ), 
    Y_coord_X_inter_BT_triangle = case_when(T_dist <= 1784 & azi_T_BT_inter_1 == azi_T_B ~ intersection_c_lx1(b0_BT,b1_BT, 0, 0, data_circle$rmax[3]*2),
  ...
  mutate(X_coord_Y_inter_AT_triangle = Y_line(b0_AT, b1_AT, Y_coord_X_inter_AT_triangle),  
         X_coord_Y_inter_BT_triangle = Y_line(b0_BT, b1_BT, Y_coord_X_inter_BT_triangle)) 
...
HBI_trees %>% 
  ...
  mutate(# because it´s coordiantes, we have to use Y_tree for x and X_tree for y 
         X_coord_Y_AB_t_implicit = implicit(b0_AB, b1_AB, X_tree, Y_tree), 
         X_coord_Y_AT_t_implicit = implicit(b0_AT, b1_AT, X_tree, Y_tree), 
         X_coord_Y_BT_t_implicit = implicit(b0_BT, b1_BT, X_tree, Y_tree) ) %>%
  ...
  # assign a tree-edge-status that calls trees with a Y higher then the respective edge-functions Y
   ...
   mutate(t_AB_status = ifelse(X_coord_Y_AB_t_implicit == 0, "on line", ifelse(X_coord_Y_AB_t_implicit > 0, "C", "D")), 
          t_AT_status = ifelse(X_coord_Y_AT_t_implicit == 0, "on line", ifelse(X_coord_Y_AT_t_implicit > 0, "A", "B")), 
          t_BT_status = ifelse(X_coord_Y_BT_t_implicit == 0, "on line", ifelse(X_coord_Y_BT_t_implicit > 0, "A", "B")), 
                                   # if both arms of triangle cross circle, use the triangle function
          t_ABT_status = case_when(inter_status_AT == "two I" & inter_status_BT == "two I" ~ p.in.triangle(Y_coord_X_inter_AT_triangle, Y_coord_X_inter_BT_triangle, Y_T, X_coord_Y_inter_AT_triangle, X_coord_Y_inter_BT_triangle, X_T, Y_tree, X_tree),
...
view(trees_and_edges %>% 
       filter(e_form == 2 & is.na(t_ABT_status)))
...
ggplot() +                             
  ...
  geom_point(data = forest_edges_HBI.man %>% 
               filter(e_form == "1"), 
             aes(Y_coord_X1_inter_AB, X_coord_Y1_inter_AB, colour = "inter 1"))+
  geom_point(data = forest_edges_HBI.man %>% 
               filter(e_form == "1" ), 
             aes(Y_coord_X2_inter_AB, X_coord_Y2_inter_AB, colour = "inter 2"))+
  facet_wrap(~plot_ID)+
...
ggplot() +  
  ...
  geom_point(data = trees_and_edges %>%
               ...
               inner_join(.,   forest_edges_HBI.man %>% 
                            ...
                            filter(n <= 1), 
                          by = "plot_ID") %>% 
              select(plot_ID, Y_coord_X1_inter_AB, Y_coord_X2_inter_AB, Y_A, Y_B, X_coord_Y1_inter_AB, X_coord_Y2_inter_AB, X_A, X_B) %>% 
              to_long(keys = c("X_name",  "Y_name"),
                      values = c( "X_value", "Y_value"),  
                      names(.)[2:5], names(.)[6:9]), 
  ...
  geom_point(data =  trees_and_edges %>% filter(e_form == "1") %>% 
               inner_join(.,   forest_edges_HBI.man %>% 
                            ...
                            filter(n <= 1), 
                          by = "plot_ID"),
             aes(x= Y_tree, y = X_tree, colour = t_AB_status))+
  
  geom_point(data = trees_and_edges %>% 
               filter(e_form == "2") %>%  
               select(plot_ID, Y_coord_X_inter_AT_triangle , Y_T , X_coord_Y_inter_AT_triangle , X_T) %>%
               to_long(keys = c("X_name",  "Y_name"),
                       values = c( "X_value", "Y_value"),  
                       names(.)[2:3], names(.)[4:5]), 
             aes(x= X_value, y = Y_value,colour = X_name))+
  ...
  geom_point(data = trees_and_edges %>% 
              ...
              to_long(keys = c("X_name",  "Y_name"),
                      values = c( "X_value", "Y_value"),  
                      names(.)[2:3], names(.)[4:5]), 
            aes(x= Y_value, y = X_value, colour = X_name))+
  ...
  geom_line(data = trees_and_edges %>% 
              ...
              to_long(keys = c("X_name",  "Y_name"),
                      values = c( "X_value", "Y_value"),  
                      names(.)[2:3], names(.)[4:5]), 
            aes(x= Y_value, y = X_value))+
  # BT line 
  geom_point(data = trees_and_edges %>% 
               filter(e_form == "2") %>%  
               select(plot_ID, Y_coord_X_inter_BT_triangle , X_T, X_coord_Y_inter_BT_triangle , Y_T) %>%
               to_long(keys = c("X_name",  "Y_name"),
                       values = c( "X_value", "Y_value"),  
                       names(.)[2:3], names(.)[4:5]), 
             aes(x= X_value, y = Y_value, colour = X_name))+
  ...
  geom_point(data =  trees_and_edges %>% filter(e_form == "2"),
             aes(Y_tree, X_tree, colour = t_ABT_status))+
...
ggplot() +  
 ...
 # AT line whne T outside cirlce
  geom_point(data = trees_and_edges %>% 
               filter(e_form == "2") %>% 
               select(plot_ID, Y_coord_X1_inter_AT, X_T, X_coord_Y1_inter_AT, Y_T) %>% 
               to_long(keys = c("X_name",  "Y_name"),
                       values = c( "X_value", "Y_value"),   
                       names(.)[2:3], names(.)[4:5]),
  ...
  geom_line(data = trees_and_edges %>% 
              filter(e_form == "2") %>% 
              select(plot_ID, Y_coord_X2_inter_AT, X_T, X_coord_Y2_inter_AT, Y_T) %>% 
              to_long(keys = c("X_name",  "Y_name"),
                      values = c( "X_value", "Y_value"),  
                      names(.)[2:3], names(.)[4:5]),
            aes(x= X_value, y = Y_value, colour = "AT"))+
  ...
  geom_point(data = trees_and_edges %>%
               filter(e_form == "2" ) %>% 
               select(plot_ID, Y_coord_X1_inter_BT, X_T, X_coord_Y1_inter_BT, Y_T) %>% 
               to_long(keys = c("X_name",  "Y_name"),
                       values = c( "X_value", "Y_value"), 
                       names(.)[2:3], names(.)[4:5]), 
  ...
  geom_line(data = trees_and_edges %>%
              filter(e_form == "2") %>% 
              select(plot_ID, Y_coord_X2_inter_BT, X_T, X_coord_Y2_inter_BT, Y_T) %>% 
              to_long(keys = c("X_name",  "Y_name"),
                      values = c( "X_value", "Y_value"),  
                      names(.)[2:3], names(.)[4:5]), 
            aes(x= X_value, y = Y_value, colour = "BT"))+
